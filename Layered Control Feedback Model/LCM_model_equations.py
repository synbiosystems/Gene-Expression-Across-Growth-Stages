...
def run_model(x0, param, loop = 'open', tspan = timepoints):
    ...
    def model_ODEs(t, x, p, loop):
        ...
        y_resources_tx = np.power(y_squared, n_gamma_resources_tx)
        y_resources_tl = np.power(y_squared, n_gamma_resources_tl)
        y_rate = np.power(y_squared, n_gamma_rate)
        ...
        Rsrc_total = Rsrc_max * y_resources_tl
        ...
        # transcription
        beta_A = beta_A * y_resources_tx * N_plasmid_low 
        beta_B = beta_B * y_resources_tx * N_plasmid_high  
        ...
        # protein folding
        k_fold_fp = k_fold_fp * (y_rate * (psi_P) + b_fold)
        k_fold_lac = k_fold_lac * (y_rate * (psi_P) + b_fold)
        k_fold_r = k_fold_r #* (y_rate * (psi_R) + b_fold)
