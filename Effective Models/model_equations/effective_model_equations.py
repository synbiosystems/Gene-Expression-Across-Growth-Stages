...
def run_model(param, degtag, tspan):
    ...
    def model_odes(x, t, param, degtag = 1):
        ...
        n_gamma = p['n_gamma'] # Exponent of gamma
        ...
        alpha = 1 - f
        delta = f**n/(1 + f**n)
        gamma_resources = np.power(gamma_base, n_gamma)
        gamma_rate = np.power(gamma_base, n_gamma)
        gamma_deg = np.power(gamma_base, n_gamma)
        ...
        d_p = d_p * delta
        d_dil = d_dil * alpha
        d_tag = d_tag * (gamma_deg + b_tag) 
       
        # Amino acid synthesis rate 
        k_syn = k_syn * gamma_rate 

        # Translation rate 
        k_tl = k_tl * gamma_resources 
        ...
        dAdt = k_lag * Pc + k_syn - k_tl * M * A - d_dil * A 
        dCdt = k_gr * C * (1 - f)
